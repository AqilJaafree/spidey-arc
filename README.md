...
`SOLANA_RPC_URL` overrides the endpoint Meteora's bin reader uses. Worth setting if you have a paid node, not required: measured at the real cadence — three scans 60s apart — the public endpoint enriched 8 of 8 pools every time, in 7–8.5s. The limit is further out than the call count suggests. Five scans back to back, about 120 calls in 25 seconds, collapsed to 8, 2, 0, 0, 0 enriched; but the pool cache cannot produce that, because its 60s TTL *and* its in-flight guard collapse concurrent callers into one upstream fetch, so the ceiling is 25 calls a minute no matter how many people are clicking. An earlier version of this paragraph called the public node unusable for a server, reasoning from the call count without measuring it. What that episode is genuinely worth remembering is how the failure looked: no error, no log, just 0 of 8 enriched and *faster* each time, because a refused read returns quickly. `/health` now reports a `degraded` list so it cannot happen silently again.
