...
function PageContent() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 md:px-6 lg:px-8">
      ...
      {best && (
        <section aria-live="polite" className="mb-10 rounded border border-border bg-card p-6 sm:p-8">
          <p className="text-sm font-medium text-muted-foreground">
            Best for {usdFull(sizeUsd)} — {best.pair.join(' / ')}{' '}
            <span className="text-xs">({best.dex})</span>
          </p>
          <p className="font-serif mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1 text-4xl md:text-5xl">
          ...
          {heroGap?.direction === 'above' && (
            <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
              This can happen when your deposit sits inside a narrower, more active range than the
              pool&apos;s full liquidity — you capture a larger share of the fees actually being
              paid than the blended headline rate implies.
            </p>
          )}
          {heroGap?.direction === 'below' && best.flags.includes('dilution') && (
            <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
              Your deposit is large relative to this pool&apos;s in-range liquidity, so you earn a
              smaller share of the fee rate than the headline number implies.
            </p>
          )}

          {data?.headlineDisagreement.disagrees && headlineBest && headlineBest.poolId !== best.poolId && (
            <p className="mt-3 max-w-2xl text-sm text-warning">
              Ranking by headline APR alone would point you at {headlineBest.pair.join(' / ')}{' '}
              instead ({aprFromBps(headlineBest.headlineAprBps)} headline) — a different pool, at
              this size.
            </p>
          )}
        </section>
      ...
      <footer className="mt-12 space-y-2 border-t border-border pt-6">
